...
from equinox._serialisation import _maybe_open
...
#####################
def _check_node(
    obj: Any, self_key: str = None, depth: int = 0.0, _print: bool = False
) -> bool:
    """
    ...
    if isinstance(obj, conatiner_types):
        if _print:
            print(f"{t}Node '{self_key}' of type: {type(obj)}")
        return True
    ...
    # Leaf node
        ...
        if _print:
            print(f"{t}Leaf '{self_key}' of type: {type(obj)}")
        return False

    # Unknown - Raise error
    else:
        raise ValueError(
            f"Unknown node '{self_key}' of type: {type(obj)}. "
...
def _build_node(
    obj: Any, key: str = None, depth: int = 0, _print: bool = False
) -> dict:
    """
...
def _build_leaf_node(obj: Any) -> dict:
    ...
    """
    # Basic info
    node_dict = {"node_type": "leaf", "type": _format_type(obj)}
...
def build_structure(
    obj: Any, self_key: str = None, depth: int = 0, _print: bool = False
):
    """
...
def serialise(path_or_file: str, obj: Any) -> None:
    """
    ...
    ----------
    path_or_file : str
        The path or file to serialise the object to.
    obj : Any
    ...
    if isinstance(path_or_file, str) and not path_or_file.endswith(".zdx"):
        path_or_file += ".zdx"
    ...
    # Modules that existed in the global namespace - Search for it
        ...
        try:
            ...
            raise AttributeError(
                f"The module/class '{'.'.join(modules)}' "
...
def deserialise(path_or_file: str):
    """
