...
from ._validation import _validate_rebuilt
...
def save(file_or_path, pytree) -> None:
    """Save supported Equinox modules and containers to a Zodiax archive.
    ...
    Unregistered callables, opaque objects, weakly typed JAX arrays, and unsupported
    JAX dtypes cannot be saved losslessly. Register stateless symbolic callables with
    :func:`register_callable`. Python integers must fit in signed 64 bits. Non-finite
    Python float and complex values are unsupported and should be stored as strongly
    typed JAX arrays when required.
    """
...
def load(file_or_path, *, like=None, custom_types=None):
    ...
    like = ExampleModule(array=jnp.zeros((2, 2)))
    ...
    constructor invariants. A class may opt into semantic post-load validation by
    defining a no-argument ``__zodiax_validate__`` method that raises when its rebuilt
    ...
    with source as file:
        try:
            with zipfile.ZipFile(file, mode="r") as archive:
                ...
                _validate_rebuilt(loaded)
                validate_links(loaded)
                # Hooks are trusted extension code but are validators, not migration
                # callbacks. Rechecking catches any structural or metadata mutation.
                definition.validate(loaded)
                return loaded
