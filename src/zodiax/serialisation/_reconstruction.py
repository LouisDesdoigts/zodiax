...
def _decode_literal(definition, custom_types, path):
    ...
    if kind == "callable":
        try:
            ...
            raise ValueError(
                f"Invalid 'callable' literal definition at {path}."
            ) from error
        return _resolve_callable(identifier, path)
...
def _build_template(definition, custom_types, path="root"):
    ...
    if kind == "equinox_state":
        entries = definition.get("entries")
        if not isinstance(entries, list):
            raise ValueError(f"State entries at {path} must be a list.")
        keys = []
        values = []
        for index, entry in enumerate(entries):
            if not isinstance(entry, dict):
                raise ValueError(f"Invalid State entry at {path}[{index}].")
