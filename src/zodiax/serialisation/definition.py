...
def _describe(value, *, static, template, path):
    ...
    if isinstance(value, eqx.nn.State):
        if type(value) is not eqx.nn.State:
            raise TypeError(
                f"{path} has unsupported Equinox State subclass "
                f"{type(value).__name__}."
            )
        try:
            state_values, state_keys = value.tree_flatten()
        except ValueError as error:
            raise TypeError(f"{path} is not a valid Equinox State.") from error
        if len(state_keys) != len(state_values):
            raise TypeError(f"{path} has inconsistent Equinox State keys and values.")

        entries = []
        for index, (key, item) in enumerate(zip(state_keys, state_values)):
            key_path = f"{path}.keys[{index}]"
            if type(key) not in (int, str):
                raise TypeError(
                    f"{key_path} is not a stable state key. Construct stateful "
                    "models with eqx.nn.make_with_state()."
                )
            entries.append(
                {
                    "key": _literal_definition(key, key_path),
                    "value": _describe(
                        item,
                        static=static,
                        template=template,
        ...
        return {"kind": "equinox_state", "version": 1, "entries": entries}
    ...
    try:
        if isinstance(value, eqx.Module):
            ...
            try:
                ...
                raise ValueError("like contains an invalid Equinox State.") from error
