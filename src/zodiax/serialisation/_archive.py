...
class _PayloadReader:
    """Bound reads to the payload member size and expose remaining bytes."""
    ...
    @property
    def remaining(self):
        """Number of archive-member bytes not yet consumed."""
        return self.size - self.consumed

    def read(self, size=-1):
        """Read at most the remaining declared payload bytes."""
        ...
        if len(data) > size:
            raise OSError("The Zodiax payload reader exceeded its declared bound.")
...
def _write_archive(file, pytree, definition):
    """Write one complete Zodiax archive to a binary file object."""
    with zipfile.ZipFile(file, mode="w", allowZip64=True) as archive:
        ...
        if len(data) > _MAX_MANIFEST_SIZE:
            raise ValueError("The generated Zodiax manifest is too large.")
        archive.writestr(_zip_info(_MANIFEST_MEMBER), data)


def _normalise_path(file_or_path):
    """Return a path with the default Zodiax suffix, or None for file objects."""
...
def _validate_manifest(manifest):
    """Validate the fixed fields required to interpret a v1 Zodiax archive."""
    if not isinstance(manifest, dict):
        raise ValueError("The Zodiax manifest must be a JSON object.")
    required = {"format", "definition", "payload", "created_with"}
    if not required.issubset(manifest):
        raise ValueError("The Zodiax manifest is missing required fields.")

    format_spec = manifest["format"]
    if not isinstance(format_spec, dict) or format_spec.get("name") != _FORMAT_NAME:
        raise ValueError("This is not a supported Zodiax archive.")
    ...
    if not isinstance(payload, dict):
        raise ValueError("The Zodiax manifest has an invalid payload description.")
    if payload.get("member") != _PAYLOAD_MEMBER:
        raise ValueError(
            "The Zodiax manifest identifies an unsupported payload member."
        )
    if payload.get("codec") != _PAYLOAD_CODEC:
        raise ValueError(f"Unsupported Zodiax payload codec {payload.get('codec')!r}.")
...
def _read_manifest(archive):
    ...
    if info.file_size > _MAX_MANIFEST_SIZE:
        raise ValueError("The Zodiax manifest is too large.")
    try:
        ...
        raise ValueError("The Zodiax manifest is not valid UTF-8 JSON.") from error
...
def _validate_members(archive):
    ...
    if len(names) != len(set(names)):
        raise ValueError("The Zodiax archive contains duplicate members.")
    if set(names) != _MEMBERS:
        raise ValueError(
            "The Zodiax archive must contain manifest.json and leaves.eqx."
        )
    for info in archive.infolist():
        if info.flag_bits & 0x1:
            raise ValueError("Encrypted Zodiax archive members are unsupported.")
        if info.compress_type != zipfile.ZIP_STORED:
            raise ValueError("Compressed Zodiax archive members are unsupported.")
...
def _validate_payload_size(archive, manifest):
    ...
    if actual != expected:
        raise ValueError("The Zodiax payload size does not match its manifest.")
