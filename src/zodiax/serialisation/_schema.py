...
def _validate_node(node, path, depth, state, ancestors, *, retained):
    ...
    try:
        ...
        if kind == "equinox_state":
            _require_keys(node, {"kind", "version", "entries"}, path)
            if type(node["version"]) is not int or node["version"] != 1:
                raise ValueError(f"Unsupported Equinox State version at {path}.")
            entries = node["entries"]
            if not isinstance(entries, list) or len(entries) > _MAX_ITEMS:
                raise ValueError(f"State entries at {path} must be a bounded list.")
            keys = []
            for index, entry in enumerate(entries):
                entry_path = f"{path}.entries[{index}]"
                if not isinstance(entry, dict):
                    raise ValueError(f"State entry at {entry_path} must be a mapping.")
                _require_keys(entry, {"key", "value"}, entry_path)
                key = entry["key"]
                if type(key) is int:
                    _validate_integer(key, f"{entry_path}.key")
                elif type(key) is str:
                    if not 0 < len(key) <= _MAX_STATE_KEY:
                        raise ValueError(f"Invalid State key at {entry_path}.")
                else:
                    raise ValueError(f"Invalid State key at {entry_path}.")
                keys.append(key)
                _validate_node(
            ...
            if len(keys) != len(set(keys)):
                raise ValueError(f"Duplicate State keys at {path}.")
            return
