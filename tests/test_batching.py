...
def test_get_batch_sizes_padding_and_shape():
    idx, total = zodiax.batching._get_batch_sizes(5, 2)
...
def test_get_batch_sizes_raises_for_too_many_batches():
    with pytest.raises(ValueError, match="nbatches"):
        zodiax.batching._get_batch_sizes(3, 4)
...
def test_jacobian_batched_matches_unbatched(jit, checkpoint):
    x = _make_x()
    j_ref, unflatten_ref = zodiax.batching.jacobian(
        _vector_fn,
        x,
        nbatches=1,
        jit=jit,
    ...
    j_batched, unflatten_batched = zodiax.batching.jacobian(
        _vector_fn,
        x,
        nbatches=2,
        jit=jit,
...
def test_hessian_batched_matches_unbatched(jit, checkpoint):
    x = _make_x()
    h_ref, _ = zodiax.batching.hessian(
        _scalar_fn,
        x,
        nbatches=1,
        jit=jit,
    ...
    h_batched, _ = zodiax.batching.hessian(
        _scalar_fn,
        x,
        nbatches=2,
        jit=jit,
...
def test_hessian_raises_for_too_many_batches():
    x = _make_x()
    with pytest.raises(ValueError, match="nbatches"):
        zodiax.batching.hessian(_scalar_fn, x, nbatches=4, jit=False)
...
def test_hessian_to_pytree_structure_and_values():
    ...
    h_tree = zodiax.batching.hessian_to_pytree(h_flat, x)
...
def test_hessian_to_pytree_shape_mismatch_raises():
    ...
    with pytest.raises(ValueError, match="H has shape"):
        zodiax.batching.hessian_to_pytree(np.zeros((2, 2)), x)
